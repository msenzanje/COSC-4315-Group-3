import re
from chat import chatbot
import os
import json

exam_folder =  './tex_exam_collection'
answer_folder = './responses'


def extract_questions(file_path):
    questions = []
    with open(file_path, 'r', encoding='utf-8') as file:
        text = file.read()

    crop_txt = re.split(r'\n1\.\s+', text, maxsplit=1) # We do this to skip the first page

    # Match numbered questions (with optional parts like (a), (b), etc.)
    pattern = r'\n(\d{1,2})\.\s*(.*?)\n(?=\d{1,2}\.|\Z)'  # Match each question block
    matches = re.findall(pattern, crop_txt[1], re.DOTALL)

    for i, content in matches:
        content = f'{i}. {content.strip()}'
        questions.append(content)

    return questions


# TEST
# questions = extract_questions("tex_exam_collection/Exam 1_Calculus 1.tex")


# Iterate through questions and store model-generated responses in a JSON file
for file in os.listdir(exam_folder):

    path =  os.path.join(exam_folder,file)
    questions = extract_questions(path)

    # Pair questions and responses
    qa_pairs = []
    for i, question in enumerate(questions):
        answer = chatbot(question) # Is this the problem?
        qa_pairs.append({
            "question_number": i + 1,
            "question": question,
            "answer": answer
        })

    print(qa_pairs)



    # # Write to JSON
    # base_filename = os.path.splitext(os.path.basename(path))[0]
    # out_file = os.path.join(base_filename,'_responses.json')
    # out_path = os.path.join(answer_folder, out_file)
    
    # with open(out_path, 'w', encoding='utf-8') as f:
    #     json.dump(qa_pairs, f, indent=4)

    # print(f'Saved to {out_path}')